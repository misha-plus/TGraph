 ...
 */
public class SameDegreeGraphBruteForcerTest {
    @Test
    public void testBrute1vertexGraph() throws Exception {
        ...
        new SameDegreeGraphBruteForcer(n, d, actual::add).brute();
        ...
        decartUnite(variants.getVariants(1, 2), variants.getVariants(2, 1))
                .forEach(edges -> {
                    DirectedPseudograph<Integer, MyEdge> g
                            = DirectedPseudographCreator.create(edges);
                    if (Util.isGraphHaveSameDegree(g) && Util.getSomeDegree(g) == 2)
                        expected.add(g);
                });
        Set<DirectedPseudograph<Integer, MyEdge>> actual = Sets.newHashSet();
        new SameDegreeGraphBruteForcer(n, d, actual::add).brute();
